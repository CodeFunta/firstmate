...
For registered GitHub projects, the provider delta adds `gh`, GitHub auth via `gh auth login`, and compatible `gh-axi`; for registered GitLab projects, it adds `glab`, GitLab auth via `glab auth login`, and compatible GitLab tooling. Local-only homes need neither forge delta.
